...
**Kalıcı profil:** sosyal/portföy bilgisi + **yüksek seviyeli içerik yetenek bayrakları**. **Malzeme teslim listesi**(MIN_5_PHOTOS vb.) **`creators` üzerinde tutulmaz**; teslim/yayında acente doğrular.
...
| `departure_city` | VARCHAR | İsteğe bağlı görünür metin |
| `departure_region` | ENUM | Pilot `ADANA`, `MERSIN` |
| `destination_cluster` | ENUM | `KAPADOKYA`, `GUNEY` vb. |
| `status` | ENUM | `tour_status` |
| `starts_at` | TIMESTAMPTZ | Başlangıç |
| `ends_at` | TIMESTAMPTZ | Bitiş |
| `transportation_included` | BOOLEAN NOT NULL DEFAULT TRUE | Ulaşım dahil (**MVP ürün kuralına uygun**, pilot: true beklenir) |
