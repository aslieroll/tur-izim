...
| `GET/PATCH` | `/creators/me` | **`capability_*` booleanları** dahil güncellenmiş profil |
...
| Metot | Yol | Açıklama |
|-------|-----|----------|
| `POST` | `/tours` | Tur oluştur — yalnızca **onaylı** acente; pilot + `estimated_tour_price`, **`creator_quota`**, **`deposit_amount`**, `tour_requirement_type` satırları ile |
| `GET` | `/tours/public` veya `/tours/open` | **Yayımlanmış**, başvuruya uygun liste |
| `GET` | `/tours/{id}` | Tekil tur detayı |
...
| Metot | Yol | Açıklama |
|-------|-----|----------|
| `POST` | `/tours/{tourId}/applications` | İçerik üreticisi **başvurur**. **`accepted_*` üçünün de `true` olması zorunlu**dır; aksi halde **başvuru kaydı oluşturulmaz** (**skor parçası değildirler** — `suitability-score.md`). UI’da **ön işaret yoktur**. |
